(ns datascript.query-v3
  (:require
    ...
    [datascript.parser :as dp :refer [BindColl BindIgnore BindScalar BindTuple
                                      Constant DefaultSrc Pattern RulesVar SrcVar Variable
                                      Not Or And Predicate PlainSymbol]]
    [datascript.btset :as btset :refer [Iter]]))
...
(defn q [q & inputs]
  (let [parsed-q (parse-query q)
    ...
    (dq/-post-process (:qfind parsed-q) (native-coll (collect-to context syms (fast-set) [(map vec)])))))
